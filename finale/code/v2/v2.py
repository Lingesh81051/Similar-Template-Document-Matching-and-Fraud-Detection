...
class ImageComparator:
    ...
    def select_roi(self):
        if self.template is None:
            self.label_status.config(text="Please select a template first")
            return
        ...
        # Resize the template image to fit within a reasonable size
        max_height = 600
        max_width = 800
        ...
        while True:
            template_display = resized_template.copy()
            if self.selected_roi is not None:
                x, y, w, h = self.selected_roi
                cv2.rectangle(template_display, (x, y), (x + w, y + h), (0, 255, 0), 2)
            self.resize_and_display_image(template_display, self.label_image2)
            cv2.imshow("Select ROI", template_display)
