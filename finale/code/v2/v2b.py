...
class ImageComparator:
    ...
    def select_roi(self):
        ...
        # Resize the claimed document image to fit within the available space while maintaining its aspect ratio
        max_height = 500
        max_width = 5000
        height, width = self.claimed_document.shape[:2]
        scale = min(max_height / height, max_width / width)
        resized_claimed_document = cv2.resize(self.claimed_document, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA)

        while True:
            document_display = resized_claimed_document.copy()
            if self.selected_roi is not None:
                x, y, w, h = self.selected_roi
                cv2.rectangle(document_display, (x, y), (x + w, y + h), (0, 255, 0), 2)
            self.resize_and_display_image(document_display, self.label_image2)
